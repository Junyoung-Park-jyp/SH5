...
소비 패턴 분석으로 여행 성향까지 알려주는 여행 비서 **여비**

## Team 5일장

| 이름   | 구분 | 역할 및 수행업무                       |
| ------ | ---- | -------------------------------------- |
| 임광영 | 팀장 | 프론트엔드, 결제 카테고리화, AI 이미지 |
| 박준영 | 팀원 | 프론트엔드, Axios, UI/UX, CSS          |
| 오승진 | 팀원 | 인프라                                 |
| 이선재 | 팀원 | 프론트엔드, Axios, UI/UX, CSS          |
| 정지환 | 팀원 | 백엔드                                 |
| 정태완 | 팀원 | 백엔드, DB, API, 인프라                |

## 개발 환경

- **언어:** Python 3.11.4, JavaScript
- **프레임워크:** Django 5.1, Vue3 // Spring Boot 3.X, React 18.2
...
**여비 홈페이지**: [여비 홈페이지](https://5illjjang.click/)  
**여비 API Page**: [여비 API Page](https://5illjjang.click/api/)

현재 저희는 배포가 되어있는 상태로, **여비** 페이지에서 개발한 기능은 모두 확인해보실 수 있습니다.  
...
물론 배포된 사이트에선 가능합니다.  
만약 테스트를 해보고 싶으시다면, 팀원 `정태완`에게 연락주시면 해당 변수 값이 담긴 `.env`파일을 개별적으로 보내드리겠습니다.
...
전체적인 흐름을 한눈에 파악하고 싶으시다면 [여비 figma](https://www.figma.com/design/KVhQnVpmke2D3pq7rM53yH/%EC%8B%A0%ED%95%9C%EC%9D%80%ED%96%89-%ED%95%B4%EC%BB%A4%ED%86%A4?t=thyBP42gwwVKKNlB-0)를 참고해주시면 감사하겠습니다.

### 1. 시작 페이지(HomeView)

![HomeView](./img/HomeView.png)

- 이 프로젝트는 금융 어플에 이식하는 형태를 생각하고 만든 프로젝트입니다.
- 그러므로 금융 어플에서 이미 회원가입과 로그인을 완료했다고 가정하고 시작합니다.
- 이식하는 형태를 생각했다면 회원가입과 로그인 기능을 보여드리는 것이 의미가 없다고 생각했고, 시연을 위해 현재는 사이트 접속시 자동으로 김신한씨로 로그인합니다.
- 4대 금융사 중 신한은행의 디자인을 차용했고, 자신의 이름과 계좌번호, 잔고를 보여줍니다.
- 이제 SOL로 여행을 클릭해줍니다.
...
- SOL로 여행의 간략한 설명을 위에 두고, 아래에는 신한은행의 제품 중 이 서비스와 연계할 수 있는 제품인 SOL트래플 체크카드, 신용카드 소개가 있습니다.
